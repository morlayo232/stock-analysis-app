...
import streamlit as st import yfinance as yf import pandas as pd import json import os from datetime import datetime from modules import calculate_indicators, calc_investment_score, TOOLTIP_EXPLANATIONS, load_stock_price from charts import plot_stock_chart, plot_rsi_macd from update_stock_database import main as update_main from news import fetch_news_headlines
...
with st.sidebar.expander("기술 지표 설명 보기"): for key, desc in TOOLTIP_EXPLANATIONS.items(): st.markdown(f"{key}: {desc}")

✅ 수동 업데이트 기능 (변경된 종목 비교 포함)

st.sidebar.markdown("### ⟳ 수동 데이터 갱신") if st.sidebar.button("Update Now", key="update_button"): with st.spinner("업데이트 중입니다... 잠시만 기다려 주세요."): try: before_df = filtered_stocks.copy() update_main() st.cache_data.clear() filtered_stocks = load_filtered_stocks()

# 변경된 종목 표시
        diff = pd.concat([before_df, filtered_stocks]).drop_duplicates(keep=False)
        st.success("업데이트가 성공적으로 완료되었습니다.")
        if not diff.empty:
            st.info(f"🔄 총 {len(diff)}개 종목이 변경되었습니다.")
            st.dataframe(diff)
