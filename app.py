...
st.image(logo_img, width=380) # 크기 조절
except Exception:
...
if ((recent['RSI_14'].iloc[i] > 65 and recent['RSI_14'].iloc[i-1] > recent['RSI_14'].iloc[i]) or (recent['종가'].iloc[i] > recent['EMA_20'].iloc[i])) and \ (recent['MACD'].iloc[i] < recent['MACD_SIGNAL'].iloc[i] and recent['MACD'].iloc[i-1] > recent['MACD_SIGNAL'].iloc[i-1]): sell_price = recent['종가'].iloc[i] sell_date = recent['날짜'].iloc[i] if '날짜' in recent.columns else recent.index[i] c1, c2 = st.columns(2) with c1: if buy_price is not None: msg = f"{buy_price:,.0f} 원" if buy_date: msg += f"\n({buy_date} 신호)" st.metric("추천 매수가", msg) else: st.metric("추천 매수가", "조건 미충족") with c2: if sell_price is not None: msg = f"{sell_price:,.0f} 원" if sell_date: msg += f"\n({sell_date} 신호)" st.metric("추천 매도가", msg) else: st.metric("추천 매도가", "조건 미충족") 매수 가격 입력 및 추천 매도가 표시 
...
profit_ratio = (recommended_sell - input_buy_price) / input_buy_price * 100 if profit_ratio >= 15: explanations.append(f"- 매수가 대비 {profit_ratio:.2f}% 이상 수익 실현 구간입니다. 단기 고수익 실현 타이밍으로 전문가들이 권장하는 매도 시점입니다.") elif profit_ratio >= 5: explanations.append(f"- 매수가 대비 약 {profit_ratio:.2f}% 수익권으로 분할 매도를 권장합니다.") elif profit_ratio > 0: explanations.append(f"- 매수가 대비 소폭 수익 상태이나 추가 상승 가능성도 있어 신중한 판단이 필요합니다.") else: explanations.append(f"- 현재 매수가 대비 손실 구간입니다. 손절 또는 모니터링 전략이 필요합니다.") if 'MACD' in df_price.columns and 'MACD_SIGNAL' in df_price.columns: macd_latest = df_price['MACD'].iloc[-1] signal_latest = df_price['MACD_SIGNAL'].iloc[-1] if macd_latest < signal_latest: explanations.append("- MACD가 Signal선 아래에 위치해 단기 하락 신호로 작용하고 있습니다.") else: explanations.append("- MACD가 Signal선을 상향 돌파해 단기 상승 모멘텀을 보여주고 있습니다.") if 'RSI_14' in df_price.columns: rsi_latest = df_price['RSI_14'].iloc[-1] if rsi_latest > 70: explanations.append("- RSI가 70 이상으로 과매수 상태이며, 조정 가능성이 있습니다.") elif rsi_latest < 30: explanations.append("- RSI가 30 이하로 과매도 상태이지만, 매도 시점에서는 신중해야 합니다.") if '거래량' in df_price.columns: recent_volume = df_price['거래량'].iloc[-1] avg_volume = df_price['거래량'].rolling(window=20).mean().iloc[-1] if recent_volume > avg_volume * 1.5: explanations.append("- 최근 거래량이 평균 대비 크게 증가하여 매도 압력이 강해지고 있음을 시사합니다.") elif recent_volume > avg_volume: explanations.append("- 거래량이 평균 이상으로 다소 매도세가 증가하는 추세입니다.") explanations.append("종합적으로, 추천 매도 가격은 기술적 지표와 매수 가격 대비 수익률, 거래량 변동성 등을 반영한 전문가 의견입니다.") explanations.append("시장 변동성 및 개인 투자 성향을 함께 고려해 신중한 투자 판단을 하시기 바랍니다.") for line in explanations: st.markdown(f"- {line}") 
...
if "RSI_14" in df_price.columns and not np.isnan(df_price['RSI_14'].iloc[-1]): rsi_now = df_price['RSI_14'].iloc[-1] if rsi_now < 30: eval_lines.append("📉 [RSI] RSI가 30 이하로 단기 과매도 상태입니다. 기술적 반등 가능성이 있지만, 펀더멘털도 함께 고려해야 합니다.") elif rsi_now > 70: eval_lines.append("📈 [RSI] RSI가 70 이상으로 단기 과매수 상태입니다. 단기 조정 가능성이 있으니 매수 타이밍을 신중히 판단하세요.") score = scored_df.loc[scored_df["종목명"] == selected, "score"].values[0] q80 = scored_df["score"].quantile(0.8) q20 = scored_df["score"].quantile(0.2) if score > q80: eval_lines.append("✅ [종합 진단] 현재 종목은 투자 매력도가 매우 높아 적극적인 매수 혹은 분할 매수를 고려할 수 있습니다.") elif score < q20: eval_lines.append("❌ [종합 진단] 투자 매력도가 낮은 편이므로, 추가 모니터링 후 진입하거나 다른 종목을 고려하는 것이 좋습니다.") else: eval_lines.append("☑️ [종합 진단] 평균 수준의 투자 매력도를 가진 종목으로, 가격 조정 시 분할 매수를 통한 장기투자가 적합합니다.") for line in eval_lines: st.markdown(f"- {line}") 
