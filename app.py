...
if fav_list:
    sidebar_fav_selected = st.sidebar.selectbox("⭐ 즐겨찾기", fav_list, key="fav_selectbox")
else:
    sidebar_fav_selected = None
...
if st.sidebar.button("⭐ 즐겨찾기 추가" if not is_fav else "★ 즐겨찾기 해제", key="fav_btn"):
    if not is_fav:
...
if df_price is None or df_price.empty:
    ...
    st.info(
        ...
        "- **RSI:** 40 아래 과매도, 60 위 과매수(과매도=반등, 과매수=조정, 단 급등/급락 장세는 예외)\n"
    ...
    if not all(col in df_price.columns for col in required_cols):
        ...
        for i in range(1, len(recent)):
            try:
                ...
                if (
                    (rsi_now < 40)
                    or (close_now < ema_now)
                    or (macd_now > signal_now)
                    or (lower_band is not None and close_now < lower_band)
                ):
                    buy_price = close_now
                ...
                if (
                    ...
                    or (close_now > ema_now)
                    or (macd_now < signal_now)
                    or (upper_band is not None and close_now > upper_band)
                ):
