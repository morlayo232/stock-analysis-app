import plotly.graph_objs as go
import pandas as pd

def plot_price_rsi_macd(df):
    fig = go.Figure()

    # 메인: 종가 + EMA
    fig.add_trace(go.Scatter(
        x=df.index, y=df['종가'], name="종가", line=dict(color='black')
    ))
    if 'EMA20' in df:
        fig.add_trace(go.Scatter(
            x=df.index, y=df['EMA20'], name="EMA(20)", line=dict(color='blue', dash='dot')
        ))

    # 골든/데드 크로스 지점 표시
    if 'EMA20' in df:
        cross = (df['종가'] > df['EMA20']).astype(int).diff()
        golden = df.index[(cross == 1)].tolist()
        dead = df.index[(cross == -1)].tolist()
        for idx in golden:
            fig.add_trace(go.Scatter(
                x=[idx], y=[df.loc[idx, '종가']],
                mode="markers+text",
                marker=dict(color='orange', size=12, symbol='triangle-up'),
                text=["골든크로스"], name="골든크로스"
            ))
        for idx in dead:
            fig.add_trace(go.Scatter(
                x=[idx], y=[df.loc[idx, '종가']],
                mode="markers+text",
                marker=dict(color='red', size=12, symbol='triangle-down'),
                text=["데드크로스"], name="데드크로스"
            ))

    fig.update_layout(
        title="가격(종가), EMA, 골든/데드크로스",
        yaxis_title="가격(원)", legend_title="지표",
        height=450
    )

    # RSI 서브차트
    fig_rsi = go.Figure()
    if 'RSI' in df:
        fig_rsi.add_trace(go.Scatter(x=df.index, y=df['RSI'], name="RSI(14)", line=dict(color='purple')))
        fig_rsi.add_shape(type='line', x0=df.index[0], x1=df.index[-1], y0=70, y1=70,
                          line=dict(color="red", width=1, dash="dash"))
        fig_rsi.add_shape(type='line', x0=df.index[0], x1=df.index[-1], y0=30, y1=30,
                          line=dict(color="blue", width=1, dash="dash"))
        fig_rsi.update_layout(title="RSI(14)", height=200, yaxis=dict(range=[0, 100]))

    # MACD 서브차트
    fig_macd = go.Figure()
    if 'MACD' in df and 'Signal' in df:
        fig_macd.add_trace(go.Scatter(x=df.index, y=df['MACD'], name="MACD", line=dict(color='green')))
        fig_macd.add_trace(go.Scatter(x=df.index, y=df['Signal'], name="Signal", line=dict(color='orange')))
        fig_macd.add_shape(type='line', x0=df.index[0], x1=df.index[-1], y0=0, y1=0,
                           line=dict(color="black", width=1, dash="dash"))
        fig_macd.update_layout(title="MACD & Signal", height=200)

    return fig, fig_rsi, fig_macd
